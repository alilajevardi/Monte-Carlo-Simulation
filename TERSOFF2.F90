...
!!---------------------------------------------------------------------
		...
		REAL*8 TEMP,DT,DT2,DTSQ,PDT,TIME1,TIME0
...
!C ----------------------------------------------------------------------    
		...
		INTEGER NALL
		INTEGER NMAX,IPAIR
		INTEGER ISAVE,IPRINT,IEQULV,IDTEMP,ICHEM_POT,IDISTRBN_FUNC
		INTEGER ITN,INH_TERMOSTATE,IEQUILIB_CHECK,IDIFUSECOFF
		INTEGER NSTEP
		INTEGER KEND,NLMAX,NREP,NATT
		INTEGER NO_GHOST_ATOM,M_CHEM_POT
!!---------------------------------------------------------------------
		PARAMETER (RCMAX=0.1 )!,NLMAX=1000000) //NLMAX=NALL*NALL
		PARAMETER (PI=3.1415926535897932384626433832795, NBMAX=5000) 
...
!       WRITE(6,*)' The program start at  ', HOUR, ' On ',TODAY
         ...
         WRITE(6,*)'***************************************************'
         WRITE(6,*)		 
         WRITE(6,*)'  This program starts at  ', TODAY 
         WRITE(6,*)
         WRITE(6,*)'***************************************************' 
         WRITE(6,*) 
...
!--The subroutine that reads the configuration file-------------------
      ...
      SUBROUTINE INCRD()
		...
		REAL*8          OLDTEMP,OLDDT,xcu,ycu,ryk0,rxk0,XXX
!---------------------------------------------------------------------
...
!----------The main subroutine that performs the MD simulation--------
    ...
    USE PARAMETERS
    INTEGER NT    ,ii,MT,FT,c
	REAL*8 E0,E1,rate,x0,y0,z0
...
if(mod(i,10*iPRINT)==0) write(6,*)i,c, e1,e0 !rate,c/real(i)*100

  IF(mod(i,5000)==0) then 
  WRITE(16,*)E0
  WRITE(17,*)E0, Es, Ebe, Ebo, Ecor, Eout, Estif
	       
			WRITE(99,333)ITN,300D0,2000D0,3000D0,5000D0
			WRITE(99,428)(RX(iI),RY(iI),RZ(iI),1,iI=1,NALL)
...
!============  STREACHING  
        ...
        StretchingITER : DO JN=1, NNB
          ...
          ENEPOT_s=ENEPOT_s+(0.5)*BETA_R1*R0**(-2)*(RIJ(JN)-R0)**2  ! STREACH        
